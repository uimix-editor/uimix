...
describe(fixAssetPath, () => {
  it("fixes asset path based on filePath and publicPath", () => {
    ...
    const newAssetPath = fixAssetPath(assetPath, filePath, outFilePath);
    expect(newAssetPath).toBe('${new URL("images/logo.png", import.meta.url)}');
  });
...
describe(fixAssetPathInHTMLTree, () => {
  it("fixes asset path based on filePath and publicPath", () => {
    ...
    fixAssetPathInHTMLTree(ast, filePath, outFilePath);
...
describe(fixAssetPathInCSS, () => {
  it("fixes asset path based on filePath and publicPath", () => {
    ...
    const newCSS = fixAssetPathInCSS(css, filePath, outFilePath);
    expect(newCSS).toBe(`
